...
// 检查文件
 ...
 public RestResponse uploadChunk(String fileMd5,int chunk,String absolutePath){
  ...
  String chunkFileFolderPath = getChunkFileFolderPath(fileMd5);
  //得到分块文件的路径
  String chunkFileName = chunkFileFolderPath + chunk;
  // 分块源文件目录 中获取 资源 比特·数组
  String mimotype = getMimeType(null);


  try {
   //将文件存储至minIO
   boolean b = addMediaFilesToMinIO(absolutePath,mimotype, bucket_videoFiles,chunkFileName);
   if(!b){
   ...
   ex.printStackTrace();
   log.debug("上传分块文件:{},失败:{}",chunkFileName,ex.getMessage());
  }
  return RestResponse.success(true);
 }
 ...
 private void clearChunkFiles(String chunkFileFolderPath,int chunkTotal){
  ...
  try {
   ...
   // minio删除逻辑 必须要 1、 删除 指令 2、 get 所有删除的文件
